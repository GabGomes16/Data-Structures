...
void partition3(int * A, int l, int r, int *i, int *j, sortperf_t *s) {
  ...
  *i = l; *j = r;
  ...
  do {
    inccmp(s, 1);
    while (*i <= r && A[*i] < x) (*i)++, inccmp(s, 1);

    inccmp(s, 1);
    while (*j >= l && A[*j] > x) (*j)--, inccmp(s, 1);

    if (*i <= *j) swap(&A[*i], &A[*j], s), (*i)++, (*j)--;
  } while (*i <= *j);
}
...
void quickSortIns(int * A, int l, int r, sortperf_t *s) { 
    ...
    inccalls(s, 1);
    if (l < j){
      if (j - l <= limite) insertionSort(A, l, j, s);
      else quickSortIns(A, l, j, s);
    }
    if (i < r){
      if (r - i <= limite) insertionSort(A, i, r, s);
